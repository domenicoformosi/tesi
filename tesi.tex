...
\\
Gli obiettivi del TSN a cui il prototipo vuole andare in contro sono:
\begin{itemize}
...
\end{figure}
L'implementazione del prototipo si base su un insieme di assunzioni:
\begin{itemize}
    ...
    \item Il prototipo sfrutta emulazione di hardware Wi-Fi 7 tramite \texttt{mac80211\_hwsim}, modulo kernel citato nella sezione \ref{sec: stack80211}. 
    \\
...
\section{Struttura}
Il modello utilizzato per il prototipo, in maniera coerente con l'obiettivo principale di voler dimostrare la fattibilità dell'infrastruttura stessa,non prevede entità centrali come CNC o CUC visti in \ref{subsec:modello_rete_centralizzata} o \ref{subsec:modello_completamente_centralizzato}, nè meccanismi di configurazione distribuiti visti in \ref{subsec:modello_completamento_distribuito}.
\\
...
\begin{itemize}
    ...
    \item FRER mediante l'implementazione sperimantale XDP FRER \cite{xdp}
\end{itemize}
...
Le interfacce di rete Wi-Fi 7 sono emulate tramite il modulo \texttt{mac80211\_hwsim} e iniettate nei diversi \textit{network namespaces} \ref{subsec:namespaces}, sulla quale verrano utilizzati i tool \texttt{wpa\_supplicant} e \texttt{hostapd} per gesire rispettivamente stazioni client - STA e access point - AP. Per un approfondimento sulla configurazione dei due tool si veda l'appendice \ref{Appendice:config-wifi}.
...
\\
Andando ad emulare un dispositivo Wi-Fi 7, nello specifico utilizzando \texttt{mac80211\_hwsim}, non si ha a disposizione un hardware virtuale dotato di code multiple, rendendo di fatto impossibile, \textit{senza ricorrere ad accorgimenti ad-hoc}, l'utilizzo di protocolli basati sul \textit{Traffic Control} come \texttt{taprio}.
\\
...
\begin{lstlisting}[
    ...
    ip netns exec bridge-repl stdbuf -oL -eL xdpfrer -m repl -i vproxy-repl-out:10 -e veth-repl:55 -e veth-repl-b:56 >/tmp/frer_repl.log 2>&1 &
\end{lstlisting}

\subsection{TAPRIO}
\label{subsec:taprio}
È stato precedentemente sottolineato come, su un dispositivo emulato tramite \texttt{mac80211\_hwsim}, non sia possibile applicare direttamente la \textit{queuing discipline} \texttt{taprio}, a causa della presenza di una singola coda di trasmissione. 
\\
...
Una volta ridiretti i pacchetti in ingresso da \texttt{wlan1} verso \texttt{vproxy-repl-in} (tramite \texttt{tc redirect}), subentra la necessità dell'azione \texttt{skbedit priority 5}, anticipata nel listato \ref{code:taprio_vproxy}.
...
\subsection{PTP}
In assenza di hardware dotato di \textit{Physical Hardware Clock} - PHC e capacità di \textit{hardware timestamping}, l'utilizzo di \texttt{ptp4l} richiede che l'interfaccia di rete sia dotata delle \textit{capabilities} \texttt{software-receive} e \texttt{software-transmit}.
\\
I dispositivi creati mediante \texttt{mac80211\_hwsim} dispongono unicamente della \textit{capability} \texttt{software-receive}. Tuttavia, come nei casi precedenti, le interfacce \texttt{veth} possiedono entrambe le \textit{capabilities}: risulta quindi sufficiente incapsulare l'interfaccia radio all'interno di una \texttt{veth} per consentirle di comunicare correttamente con il processo \texttt{ptp4l}.
\\
...
Per il primo test, si faccia riferimento al listato \ref{code:repl-elim}. Una prima fase di inizializzazione registra il valore di partenza del contatore \texttt{xdp\_xmit} sulle due interfacce \texttt{veth-repl} e \texttt{veth-repl-b} (che rappresenta il numero di pacchetti instradati verso le rispettive interfacce dal layer XDP). Successivamente, viene rilevato il valore iniziale dei contatori \textit{Passed} e \textit{Dropped} forniti dal programma XDP.
\\ 
...
\begin{lstlisting}[
    ...
    replicazione  percorso A (veth-repl)   : 10
    replicazione  percorso B (veth-repl-b) : 10
...
\begin{lstlisting}[
    ...
    # MAC address del bridge-repl: 5e2fe8.fffe.b2a260 
\end{lstlisting}
...
In conclusione, l'esito positivo dei test condotti permette di validare l'architettura del prototipo nella sua interezza. Nonostante le molteplici sfide implementative derivanti dai vincoli dell'ambiente virtualizzato --- quali l'assenza di code di trasmissione multiple nell'emulatore \texttt{mac80211\_hwsim}, l'impossibilità di sfruttare l'\textit{hardware timestamping} per la convergenza del protocollo PTP e la necessità di ricorrere a complessi incapsulamenti logici per far coesistere l'elaborazione eBPF/XDP con il \textit{Traffic Control} --- i risultati dimostrano la concreta fattibilità dell'estensione dei meccanismi TSN su collegamenti Wi-Fi 7.
\\
È ragionevole affermare che la trasposizione di questo \textit{proof of concept} su dispositivi di rete fisici, dotati di supporto nativo alle specifiche Wi-Fi 7 e TSN, farebbe decadere la quasi totalità dei \textit{workaround} architetturali resisi necessari in questa sede. L'impiego di NIC (\textit{Network Interface Card}) reali, provviste di \textit{Physical Hardware Clock} indipendenti, code multiple fisiche e implementazioni hardware del sottolivello MAC per la \textit{frame preemption}, non solo snellirebbe drasticamente la configurazione dello stack di rete, ma permetterebbe al sistema di esprimere appieno il proprio potenziale deterministico, confermando la solidità della convergenza IT/OT per gli scenari industriali di nuova generazione.
...
È inoltre possibile specificare, in fase di creazione, il numero di code di trasmissione e ricezione (\texttt{numtxqueues}/\texttt{numrxqueues}), caratteristica sfruttata nel prototipo per emulare il supporto a code multiple non disponibile sulle interfacce radio emulate. Le interfacce \texttt{veth} supportano inoltre l'offloading di programmi eBPF/XDP e possono essere utilizzate in combinazione con il sottosistema \texttt{tc} per redirigere e manipolare il traffico tra namespace differenti.
