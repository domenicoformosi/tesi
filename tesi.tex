...
\newcommand{\hwsim}{\mbox{\texttt{mac80211\_hwsim}}}
...
\begin{itemize}
    ...
    \item Le tecniche di isolazione della rete utilizzate vincolano l'intera infrastruttura ad avere un unico kernel ed un unico dominio di clock
\end{itemize}
...
\end{itemize}
La topologia di rete del prototipo, mostrata in figura \ref{fig:topologia_prototipo},  prevede che i collegamenti mediante Wi-Fi 7 siano quelli tra il \texttt{talker} e \texttt{bridge-repl}, ed il \texttt{listener} ed il \texttt{bridge-elim}, i due bridge sono connessi tra loro tramite un \texttt{veth} \ref{subsec:veth}.

Le interfacce di rete Wi-Fi 7 sono emulate tramite il modulo \hwsim e iniettate nei diversi \textit{network namespaces} \ref{subsec:namespaces}, sulla quale verranno utilizzati i tool \texttt{wpa\_supplicant} e \texttt{hostapd} per gestire rispettivamente stazioni client - STA e access point - AP. Per un approfondimento sulla configurazione dei due tool si veda l'appendice \ref{Appendice:config-wifi}.
...
\\
Infine, è importante sottolineare i vincoli intrinseci nati dall'emulazione dell'hardware Wi-Fi 7: una NIC Wi-Fi 7 fisica può esporre diverse code hardware sulle quali il kernel andrà a depositare i pacchetti, e sarà poi la NIC stessa a implementare le funzioni del livello MAC e del livello fisico, come ad esempio la \textit{frame preemption}. La presenza di code multiple, come discusso nel capitolo \ref{chap:cap1}, è fondamentale per le tecniche di schedulazione del traffico, poiché permette di suddividerlo in diverse classi gestite secondo logiche deterministiche.
\\
Andando ad emulare un dispositivo Wi-Fi 7, nello specifico utilizzando \hwsim, non si ha a disposizione un hardware virtuale dotato di code multiple, rendendo di fatto impossibile, \textit{senza ricorrere ad accorgimenti ad-hoc}, l'utilizzo di \textit{qdisic} come \texttt{taprio}.
\\
Inoltre, non sarà possibile implementare le tecniche di \textit{frame preemption} viste in \ref{subsec:frame_prempt}, in quanto il dispositivo emulato utilizza dei loopback virtuali per comunicare e non possiede un livello MAC effettivo (non sono disponibili i sottolivelli MAC Merge Layer, pMAC ed eMAC).
\\
...
se il programma XDP riceve un \texttt{sk\_buff}\footnote{La struttura dati principale utilizzata nel kernel Linux per rappresentare un pacchetto di rete.} non lineare, tenta di ripristinarlo richiamando il metodo \texttt{netif\_skb\_check\_for\_xdp(pskb, xdp\_prog)}; in caso di fallimento, il pacchetto subisce un \texttt{drop}.
\\
...
\\
Va tuttavia osservato che l'assenza di \textit{frame preemption} non è una limitazione dell'emulatore \hwsim, bensì un vincolo intrinseco delle interfacce radio in generale.
